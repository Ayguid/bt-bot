...
const IndicatorUtils = {
    ...
    earlyMomentumDetection: (prices, currentPrice, lookback = 5) => {
        if (prices.length < lookback) return false;
        const recentPrices = prices.slice(-lookback);
        const avgPrice = recentPrices.reduce((sum, p) => sum + p, 0) / lookback;
        const momentum = (currentPrice - avgPrice) / avgPrice;
        return momentum > 0.015; // 1.5% above average
    }
};
...
class MarketAnalyzer {
    ...
    static calculateScores(analysis) {
        ...
        // New early momentum detection
        const earlyMomentum = IndicatorUtils.earlyMomentumDetection(
            analysis.candles.slice(-20).map(c => c[4]),
            analysis.currentPrice
        );
        if (earlyMomentum) buyScore += INDICATOR_WEIGHTS.earlyMomentum;
        
        // Stronger trend confirmation requirement
        const trendConfirmed = (
            analysis.macdAnalysis?.isBuilding && 
            analysis.aoAnalysis?.isBuilding && 
            analysis.rsiAnalysis?.isRising
        );
        if (trendConfirmed) buyScore += INDICATOR_WEIGHTS.trendConfirmation;
        
        // Volume must confirm price movement
        const volumeConfirmed = (
            analysis.volumeIncrease > 10 && 
            analysis.candleAnalysis?.volumeTrend === "INCREASING"
        );
        if (volumeConfirmed) buyScore += INDICATOR_WEIGHTS.volumeConfirmation;
        ...
        // Trend-based adjustments
        if (analysis.candleAnalysis?.priceTrend === "BULLISH") {
            buyScore *= 1.1;  // Reduced from 1.2
            sellScore *= 0.9; // Increased from 0.8
        } else if (analysis.candleAnalysis?.priceTrend === "BEARISH") {
            buyScore *= 0.9;  // Increased from 0.8
            sellScore *= 1.1; // Reduced from 1.2
        }
    ...
    static generateSignal(buyScore, sellScore, priceTrend) {
        // Increased thresholds for more conservative signals
        const TREND_ADJUSTMENT = {
            BULLISH: { buy: 1.0, sell: 1.2 },  // Reduced bullish bias
            BEARISH: { buy: 1.2, sell: 1.0 },    // Reduced bearish bias
            SIDEWAYS: { buy: 1.2, sell: 1.2 }    // More conservative in sideways
        };
        
        const adj = TREND_ADJUSTMENT[priceTrend];
        const BUY_THRESHOLD = 6 * adj.buy;       // Increased from 5
        const STRONG_BUY_THRESHOLD = 10 * adj.buy; // Increased from 8
        const SELL_THRESHOLD = 6 * adj.sell;     // Increased from 5
        const STRONG_SELL_THRESHOLD = 10 * adj.sell; // Increased from 8

        // Require stronger confirmation and consider opposing scores
        if (buyScore >= STRONG_BUY_THRESHOLD && sellScore < 4) return "STRONG_BUY";
        if (buyScore >= BUY_THRESHOLD && sellScore < 3) return "BUY";
        if (sellScore >= STRONG_SELL_THRESHOLD && buyScore < 4) return "STRONG_SELL";
        if (sellScore >= SELL_THRESHOLD && buyScore < 3) return "SELL";
        return "HOLD";
    }
